...
def user_list(request):
    ...
    participants = User.objects.prefetch_related("skills", "owned_projects").order_by("-id")
...
def skill_suggestions(request):
    ...
    skills = skills.order_by("name")[:10]
...
def add_user_skill(request, user_id):
    ...
    if not skill_id and not name:
        return JsonResponse(
            {"status": "error", "message": "Передайте skill_id или name."},
            status=400,
        )
